...
static bool ESP8266_WaitResponse(const char *expected, uint32_t timeout_ms)
{
	...
	while (HAL_GetTick() - tick_start < timeout_ms)
	{
		if (ESP_Rx_Complete == true)
		{
			...
			if (strstr((char *)ESP8266_RxBuff, expected) != NULL)
			{
				ESP_Rx_Complete = false;
				return true;
			}

			if (strstr((char *)ESP8266_RxBuff, "ERROR") != NULL || strstr((char *)ESP8266_RxBuff, "FAIL") != NULL)
			{
				ESP_Rx_Complete = false;
				return false;
			}
...
static uint16_t ESP8266_UnescapePayload(const char *escaped_payload, char *raw_payload, uint16_t raw_payload_size)
{
	uint16_t read_index = 0;
	uint16_t write_index = 0;

	while (escaped_payload[read_index] != '\0' && write_index < raw_payload_size - 1)
	{
		if (escaped_payload[read_index] == '\\' &&
				(escaped_payload[read_index + 1] == '"' ||
				 escaped_payload[read_index + 1] == ',' ||
				 escaped_payload[read_index + 1] == '\\'))
		{
			read_index++;
		...
		raw_payload[write_index++] = escaped_payload[read_index++];
	}

	raw_payload[write_index] = '\0';
	return write_index;
}
...
void ESP8266_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : ESP AT module reset pin */
...
{
	return HAL_UART_Transmit_IT(&huart3, (uint8_t *)str, strlen(str)) == HAL_OK;
}
...
bool ESP8266_Cmd(const char *cmd)
{
	...
	if (ESP8266_SendString(buf) == false)
	{
		printf(">>> AT TX busy or error, len=%d.\r\n", strlen(buf));
		return false;
	}
...
bool ESP8266_SoftReset_WaitReady(void)
{
	...
	return ESP8266_WaitResponse("ready", 8000);
}

bool ESP8266_ConfigUARTNoFlow(void)
{
	if (ESP8266_Cmd("AT+UART_CUR=115200,8,1,0,0") == false)
	{
		return false;
	}
...
bool ESP8266_ConfigWiFiDefaults(void)
{
	ESP8266_Cmd("AT+CWQAP");
	ESP8266_WaitResponse("OK", 2000);

	if (ESP8266_Cmd("AT+CWAUTOCONN=0") == false || ESP8266_WaitResponse("OK", 2000) == false)
	{
		return false;
	}

	if (ESP8266_Cmd("AT+CWCOUNTRY=1,\"CN\",1,13") == false || ESP8266_WaitResponse("OK", 2000) == false)
	{
		return false;
	}
...
bool ESP8266_SetNetMode(NetModeTypeDef mode)
{
	...
	{
		if (ESP8266_Cmd(cmd) == true && ESP8266_WaitResponse("OK", 3000) == true)
		{
			return true;
		}
...
bool ESP8266_ScanWifi(const char *ssid)
{
	char cmd[128];
	uint32_t tick_start;
	...
	while (HAL_GetTick() - tick_start < 10000)
	{
		if (ESP_Rx_Complete == true)
		{
			...
			if (strstr((char *)ESP8266_RxBuff, "+CWLAP:") != NULL)
			{
				ESP_Rx_Complete = false;
				return true;
			}
...
ConnWifiStateTypeDef ESP8266_ConnectWifi(const char *ssid, const char *password)
{
	...
	tick_start = HAL_GetTick();
	while (HAL_GetTick() - tick_start < 60000)
	{
		if (ESP_Rx_Complete == true)
		{
			/* Ensure that the ESP8266_RxBuff ends with '\0' to prevent string operation overflow */
			...
			if (strstr((char *)ESP8266_RxBuff, "OK") != NULL || strstr((char *)ESP8266_RxBuff, "WIFI GOT IP") != NULL)
			{
				return CONNECT_OK;
			}
			else if (strstr((char *)ESP8266_RxBuff, "+CWJAP:1") != NULL || strstr((char *)ESP8266_RxBuff, "CWJAP:1") != NULL)
			{
				return CONNECT_TIMEOUT;
			}
			else if (strstr((char *)ESP8266_RxBuff, "+CWJAP:2") != NULL || strstr((char *)ESP8266_RxBuff, "CWJAP:2") != NULL)
			{
				return PASSWORD_ERROR;
			}
			else if (strstr((char *)ESP8266_RxBuff, "+CWJAP:3") != NULL || strstr((char *)ESP8266_RxBuff, "CWJAP:3") != NULL)
			{
				return TARGET_AP_NOT_FOUND;
			}
			else if (strstr((char *)ESP8266_RxBuff, "+CWJAP:4") != NULL || strstr((char *)ESP8266_RxBuff, "CWJAP:4") != NULL || strstr((char *)ESP8266_RxBuff, "FAIL") != NULL)
			{
				return CONNECT_FAIL;
			}
...
bool ESP8266_MQTTPublish(uint8_t conn_id, const char *topic, const char *payload, MQTTQosTypeDef qos, bool enable_retain)
{
	...
	raw_payload_len = ESP8266_UnescapePayload(payload, raw_payload, sizeof(raw_payload));

	cmd_len = snprintf(cmd, sizeof(cmd), "AT+MQTTPUBRAW=%d,\"%s\",%d,%d,%d", conn_id, topic, raw_payload_len, qos, enable_retain);
	if (cmd_len < 0 || cmd_len >= sizeof(cmd))
	{
		printf(">>> MQTT publish raw header too long, len=%d.\r\n", cmd_len);
		return false;
	}
	...
	if (ESP8266_WaitResponse(">", 3000) == false)
	{
		printf(">>> MQTT publish raw prompt timeout.\r\n");
		return false;
	}

	if (ESP8266_SendString(raw_payload) == false)
	{
		printf(">>> MQTT publish raw data TX failed.\r\n");
		return false;
	}

	return ESP8266_WaitResponse("+MQTTPUB:OK", 5000);
}
